    ...
    return BlocConsumer<PharmacyHomeCubit,PharmacyHomeStates>(
      ...
      (context, state) {
        ...
        return Scaffold(
          ...
          ListView.separated(
            shrinkWrap: true,
            itemBuilder: (context, index) =>buildMedicalhistoryList(model: cubit.Request[index],context: context),
            itemCount: cubit.Request.length,
        ...
        child: Row(
          children: [
            SizedBox(
              ...
              child: Column(
                children: [
                  Text("Order Cost: ${model.totalcost.toString()}",style: const TextStyle(fontSize: 20,fontWeight: FontWeight.bold,color: Colors.white)),
                  const SizedBox(
                    height: 5,
                  ),
          ...
          child: Column(
            children: [
              Container(
                ...
                child: Column(
                  children: [
                    Text("Order id: ${model.cartid.toString()}",style: const TextStyle(fontSize: 22,fontWeight: FontWeight.bold,color: Colors.white)),
                    Text("phone number: ${model.phone.toString()}",style: const TextStyle(fontSize: 22,fontWeight: FontWeight.bold,color: Colors.white)),
                    Text("location: ${model.location.toString()}",style: const TextStyle(fontSize: 22,fontWeight: FontWeight.bold,color: Colors.white,overflow: TextOverflow.clip)),
                    Text('${DateTime.fromMillisecondsSinceEpoch(int.parse(model.date.toString().substring(18,28))*1000)}',style: const TextStyle(fontSize: 22,fontWeight: FontWeight.bold,color: Colors.white)),
                  ],
                ),
              ),
              ...
              SizedBox(height: 20,),
              DefaultButton(text: "Delivered ", function: () {
              },)
